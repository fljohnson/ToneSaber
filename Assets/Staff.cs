...
public class Staff {
	...
	public static Scorepartwise Load(string path) {
		         try {
             XmlSerializer serializer = new XmlSerializer (typeof(Scorepartwise));
             using(FileStream stream = new FileStream(path, FileMode.Open)) {
                  return serializer.Deserialize(stream) as Scorepartwise;
             }
         } catch (Exception e) {
              UnityEngine.Debug.LogError ("Exception loading song '"+path+"': " + e);
